                ...
                double step, double min)
{
	...
	{
		const auto x = min + (i + 1) * step;
		stream << "(" << x << ", " << complexValuedVector[i] << ") ";
	}
}
...
void plot_the_frequency_response(const std::map<double, double>& frequency_response, const std::string& file_name)
{
